...
filehandle_t* FS_OpenFile( const char *filename, char* mode  )
{
	...
	#else
	int zindex = zip_name_locate(APKArchive, netpath, 0);
	if (zindex < 0) {
		LOGE("ERROR: zip_name_locate: %s\n", netpath);
		return NULL;
	}

	struct zip_stat zstat;
	zip_stat_init(&zstat);
	if (zip_stat_index(APKArchive, zindex, 0, &zstat) != 0) {
		LOGE("ERROR: zip_stat_index: %s\n", netpath);
		return NULL;
	}

	struct zip_file* zfile = zip_fopen_index(APKArchive, zindex, 0);
	if (zfile == NULL) {
		LOGE("ERROR: zip_fopen_index: %s\n", netpath);
		return NULL;
	}
	#endif
	...
	hFile->filesize = end;
	#else
	hFile->filesize = zstat.size;

	if (hFile->filesize < 0)
	{
		LOGE("Didn\'t get information about zipped file; size was %i", hFile->filesize);
		return NULL;
	}
	#endif
	...
	#else
	if (zip_fread(zfile, hFile->filedata, hFile->filesize) != hFile->filesize) {
		LOGE("ERROR: zip_fread: %s\n", netpath);
		zip_fclose(zfile);
		return NULL;
	}
